class SliderController {
  ...
  _updateSpans(slider, spans) {
    spans.forEach((span) => {
      ...
      } else if (mode && !isNaN(parseFloat(mode))) {
        // Fixed multiplier
        const totalDigits = String(parseInt(maxValue)).length;
        const scaledValue =
          ((value - minValue) / (maxValue - minValue)) * parseFloat(mode);

        // Split integer and fractional parts
        const [intPart, fracPartRaw] = scaledValue
          .toFixed(fixedValue)
          .split(".");
  ...
  _handleSliderScroll(event, slider) {
    ...
    slider.value = Math.max(
      slider.min,
      Math.min(slider.max, parseFloat(slider.value) - step),
    );
  ...
  _addSliderListeners(slider, spans) {
    ...
    if (slider.id !== "progress-bar")
      slider.addEventListener("wheel", (event) =>
        this._handleSliderScroll(event, slider),
      );
  }
